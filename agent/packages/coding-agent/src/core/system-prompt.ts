...
function buildTaskDiscoverySection(taskText: string, cwd: string): string {
	try {
		...
		if (newFilenameHits.length > 0) {
			sections.push("\nFILES MATCHING BY NAME (inspect first — edit only if an explicit acceptance criterion requires it):");
		...
		if (criteriaCount > 0) {
			...
			if (criteriaCount <= 2) {
				sections.push("Small-task signal: prefer a single-file surgical edit. Edit a second file only when an explicit criterion names it.");
			} else if (concentrated) {
				sections.push(
					"Many criteria but keywords concentrate in one file (see KEYWORD CONCENTRATION): apply all changes there in one pass; open other files only if a criterion is still unmet.",
				);
			} else if (criteriaCount >= 3) {
				sections.push(`Multi-file task: edit only the files where a specific acceptance criterion or named path requires a change. Do not speculatively touch sibling files — surplus edits reduce the score.`);
			}
		}
