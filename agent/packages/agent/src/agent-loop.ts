...
async function runLoop(
	...
	const recordSuccessfulFileMutation = async (targetPath: string): Promise<void> => {
		...
		// Only count explicitly required files (from task text) as unedited targets.
		// Dynamically-discovered siblings are candidates, not requirements — including
		// them in the breadth nudge caused speculative multi-file edits that inflated
		// the diff denominator without adding matched lines.
		const uneditedTargets = expectedFiles.filter((f: string) => !wasEdited(f));
		let breadthHint = "";
		if (consecutiveEditsOnSameFile >= 5 && uneditedTargets.length > 0) {
			breadthHint = ` STOP editing \`${normTarget}\` — you have made ${consecutiveEditsOnSameFile} consecutive edits on it. ${uneditedTargets.length} explicitly required file(s) still need edits: ${uneditedTargets
				.slice(0, 6)
				.map((f: string) => `\`${f}\``)
				.join(", ")}. Move to the next file NOW.`;
		} else if (uneditedTargets.length > 0) {
			breadthHint = ` ${uneditedTargets.length} explicitly required file(s) still need edits: ${uneditedTargets
				.slice(0, 6)
				.map((f: string) => `\`${f}\``)
				.join(", ")}. Move to the next required file.`;
		}
