...
2. **Discover only what the task names.** Run `grep` for exact symbols/strings quoted in the task before any edits. Do not run broad directory scans or sibling sweeps — touching files the reference did not touch inflates the diff denominator without scoring any matches.
...
4. **Precision-first editing.** Edit only files where an acceptance criterion or explicitly named path requires a change. Do not speculatively edit sibling files. Surplus lines in your diff always reduce your score — a smaller, exact diff outscores a larger, approximate one.
...
7. **Sibling files only when explicitly required.** Do not run `ls` or sibling scans after edits unless the task names additional files or a criterion is still unmet. Unsolicited sibling edits add surplus lines that hurt the score.
